<script>
	...
	import { galleryElements } from '../../utils/galleryElements';
	let { data } = $props();
	let grid;
	let Muuri;
	let fullData = [];
	...
	// 			data.collection.map(async (artwork) => {
	// 				const id = artwork.sourceId;
	// 				const req =
	// 					artwork.source === 'artic'
...
<div class="container mx-auto px-4 py-8">
	<h1 class="mb-8 font-sans text-2xl font-bold">My Collection</h1>
	...
	{:else}
		<div class="grid" id="grid">
			{#each data.collection as artwork}
				<div class="grid-item">
					<div
						class="item-content rounded-lg border bg-white shadow-sm transition-shadow hover:shadow-md"
					>
						<div class="relative">
							...
							{:else}
								<div
									class="flex h-48 w-full items-center justify-center rounded-t-lg bg-gray-200 text-gray-500"
								>
									No Image Available
								</div>
							{/if}
						...
						<div class="p-4">
							<h2
								class="mb-2 line-clamp-2 font-sans font-semibold text-gray-900"
								title={artwork.title}
							>
								{artwork.title}
							</h2>
							...
							{/if}
							<p class="mt-2 font-sans text-xs text-gray-500">
			...
			{/each}
			{#each Object.entries(galleryElements) as [category, items]}
				<h2>{category}</h2>
				{#each Object.entries(items) as [itemName, itemDetails]}
					<div class="grid-item">
						<div
							class="item-content rounded-lg border bg-white shadow-sm transition-shadow hover:shadow-md"
						>
							<div class="relative">
								...
								<div class="p-4">
									<h2 class="mb-2 line-clamp-2 font-sans font-semibold text-gray-900">
										{itemDetails.title}
									</h2>
								</div>
							</div>
